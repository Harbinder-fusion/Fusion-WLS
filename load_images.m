...
function [I,ANI] = load_images(path, reduce)
...
for i = 1:N
  ...
  kappa = 30;
  option = 1;
R=anisodiff(I(:,:,1,i), niter, kappa, lambda, option);
G=anisodiff(I(:,:,1,i), niter, kappa, lambda, option);
B=anisodiff(I(:,:,1,i), niter, kappa, lambda, option);
TSD=cat(3,R,G,B);
ANI(:,:,:,i) = TSD;
end
